...
class ProductController extends Controller
{
    ...
    public function create()
    {
        ...
        if (isset($_POST['submit'])) {
            ...
            $product->hot = isset($_POST['hot']) ? 1 : 0; // Kiểm tra xem sản phẩm có nổi bật không
            $product->status = $_POST['status'];
            $product->created_at = date('Y-m-d H:i:s'); // Lấy ngày và giờ hiện tại
            $product->updated_at = date('Y-m-d H:i:s'); // Lấy ngày và giờ hiện tại
    
           // Kiểm tra nếu trường avatar không trống và file được tải lên thành công
            if (!empty($_FILES['avatar']['name']) && $_FILES['avatar']['error'] === UPLOAD_ERR_OK) {
                ...
                // Gán đường dẫn file ảnh vào trường avatar của sản phẩm
                $product->avatar = $avatar;
            }

    
            // Nếu không có lỗi, tiến hành lưu dữ liệu vào CSDL
            if ($product->insert()) {
        ...
        // Hiển thị view để tạo mới sản phẩm
    ...
    public function update()
    {
        ...
        if (isset($_POST['submit'])) {
            ...
            if (empty($this->error)) {
                ...
                $product_model->id_supplier = $id_supplier;
